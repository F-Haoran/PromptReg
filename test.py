...
class TestTaskDataset(Dataset):
    def __init__(self, data_root, task_name, target_size=TARGET_SIZE):
...
def evaluate_task(model, label_warper, data_root, task_name, device, mind_loss, grad_loss):
    dataset = TestTaskDataset(data_root=data_root, task_name=task_name)
    if len(dataset) == 0:
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=f'Testing {task_name}'):
            ...
            warped, flow, _, _ = model(moving, fixed, task_id, is_training=False)
...
def main():
    ...
    model = PromptReg(task_total_number=5).to(device)
